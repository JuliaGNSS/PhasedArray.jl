...
function manifold(ant_pos::Array{Float64, 2}, f_0, ampl = doa -> 1, c₀ = 299_792_458)
    λ = c₀ / f_0
    doa -> _get_steer_vec(doa, ant_pos, λ, ampl)
end

function _get_steer_vec(doa, ant_pos::Array{Float64, 2}, λ, ampl)
    cis.(2 * π / λ * transpose(ant_pos) * doa) .* ampl(doa)
end

function _get_steer_vec(doa::Spherical, ant_pos::Array{Float64, 2}, λ, ampl)
    _get_steer_vec(CartesianFromSpherical()(doa), ant_pos, λ, ampl)
end
...
function manifold(lut::Array{Complex{Float64}, 3}, interpolation = Constant(), max_el = π)
    ...
    doa -> _get_steer_vec(doa, itp, res_ϕ, res_θ, num_θs, num_ants, max_el, expansion_length)
end
...
function expand(lut, num::Int)
    ...
    plot_surface((1:size(lut_expanded,2)) * ones(size(lut_expanded,3))', ones(size(lut_expanded,2)) * (1:size(lut_expanded,3))', angle.(lut_expanded[4,:,:]) * 180 / pi)


    lut_expanded
end

function _get_steer_vec(doa::Spherical, itp_lut, res_ϕ, res_θ, num_θs, num_ants, max_el, expansion_length)
...
function _get_steer_vec(doa, itp_lut, res_ϕ, res_θ, num_θs, num_ants, max_el, expansion_length)
    _get_steer_vec(SphericalFromCartesian()(doa), itp_lut, res_ϕ, res_θ, num_θs, num_ants, max_el, expansion_length)
end
...
function norm_manifold(lut::Array{Complex{Float64}, 3})
    num_ants = size(lut, 1)
    steer_vecs_powers = sum(abs2.(lut), dims = 1);
...
function test_lut_correctness(lut::Array{Complex{Float64}, 3}, max_el)
    ...
    zenith_angle_std = mean(std(angle.(lut[:,1,:]), dims = 1))
    horizont_angle_std = mean(std(angle.(lut[:,near_horizont_index,:]), dims = 1))
