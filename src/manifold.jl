...
function manifold(ant_pos::Array{Float64, 2}, f_0, ampl = doa -> 1, c₀ = 299_792_458)
  λ = c₀ / f_0
  doa -> copy(transpose(cis.(2 * π / λ * transpose(doa) * ant_pos))) .* ampl(doa);
end
...
function manifold(lut::Array{Complex{Float64}, 3}, interpolation = Constant(), max_el = π)
  ...
  lut_expanded = Array{Complex{Float64}, 3}(undef, num_ants, num_ϕs, num_θs + 2)
...
function _get_steer_vec(doa, itp_lut, res_ϕ, res_θ, num_θs, num_ants, max_el)
  _get_steer_vec(SphericalFromCartesian()(doa), itp_lut, res_ϕ, res_θ, num_θs, num_ants, max_el)
end
...
Base.transpose(sph::Spherical) = copy(transpose(CartesianFromSpherical()(sph)))
