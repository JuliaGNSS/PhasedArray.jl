@testset "Pattern" begin
    get_steer_vec = manifold(0.1904 / 4 * SVector(SVector(1, 1, 0), SVector(-1, 1, 0), SVector(1, -1, 0), SVector(-1, -1, 0)), 1575420e3)
    pattern = Pattern(get_steer_vec, norm, num_az = 360, num_el = 91, max_el = π / 2)
...
@testset "Pattern3D" begin
    get_steer_vec = manifold(0.1904 / 4 * SVector(SVector(1, 1, 0), SVector(-1, 1, 0), SVector(1, -1, 0), SVector(-1, -1, 0)), 1575420e3)
    pattern = Pattern3D(get_steer_vec, norm, num_az = 360, num_el = 181, max_el = π)
