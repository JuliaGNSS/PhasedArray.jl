@testset "Ideal manifold" begin
    ...
    c₀ = 299_792_458
    λ = c₀ / f_0
    manifold = @inferred IdealManifold(f_0, SVector(0,0,0))
    @test typeof(manifold) <: AbstractManifold{1}
    @test @inferred(get_steer_vec(manifold, SVector(0,0,1))) == 1.0
    @test @inferred(get_steer_vec(manifold, SVector(0,1,0))) == 1.0
    ...
    @test all(abs.(rem.(positions[1,:] .- π/2, 1π, RoundNearest)) .≈ 0)
    @test all(abs.(rem.(positions[2,:] .- π/2, 1π, RoundNearest)) .≈ 0)
    ...
    @test all(rem.(positions[1,:] .- π/2, 1π, RoundNearest) .≈ 0)
