@testset "Antenna" begin
  ...
  @testset "LUT manifold" begin
    ...
    @test norm(@inferred(steer_vec(Spherical(SVector(1, 1, 1))))) ≈ sqrt(num_ants)

    @test @inferred(steer_vec(Spherical(SVector(0,0,1)))) ≈ [1, 1, 1, 1]

    @test @inferred(steer_vec(Spherical(SVector(0,1,0)))) ≈ [1im, 1im, -1im, -1im]

    @test @inferred(steer_vec(Spherical(SVector(1,0,0)))) ≈ [1im, -1im, 1im, -1im]
  end
  ...
  @testset "LUT manifold interpolated" begin
    ...
    @test isapprox(norm(@inferred(steer_vec(Spherical(SVector(1, 1, 1))))), sqrt(num_ants); rtol = 1e-4)
  ...
  @testset "Example LUT manifold" begin
    ...
    @test norm(@inferred(steer_vec(Spherical(SVector(0,0,1))))) < sqrt(num_ants)
  end
